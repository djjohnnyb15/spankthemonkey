...
try:
    ...
    xbmcgui.Dialog().ok("spankthemonkey Import Failure", "Failed to import needed module",
                        "[COLOR red]%e[/COLOR]", "") % e
...
twit_url = 'https://script.google.com/macros/s/AKfycbyFWBTxesxuR9voG2xNtnN0L4RwghdwPZxbrCZHBesogs5yFKLM/exec?552243381116878849'

addon_id = 'plugin.video.spankthemonkey'
...
def Menu():
    ...
    if announce:
        ...
        addon.add_directory({'mode': 'twitter', 'url': twit_url}, {'title': '[COLOR blue]Twitter[/COLOR]'}, img=twit_image ,fanart=twit_fan, total_items=0)


def CONTENT(url, alt, favourite=False):
    Vitems = []
    fanart = ''
    folder_fanart = ''
    html = GetUrl(url)
    ...
    if folder_items:
        for folder_name, thumb, url, folder_fanart in folder_items:
            ...
            Vitems.append(['CONTENT', url, folder_name, '[COLOR green][B]'+folder_name+'[/COLOR][/B]', thumb, folder_fanart])
...
class TwitterWindow(xbmcgui.WindowXMLDialog):
...
def showTextDialog(caption,text):
    ...
    win = TwitterWindow('twitter-window.xml',path,'Default',caption=caption,text=text)
    win.doModal()
    del win

def twit(url):
    news, text = utils.TWITTER(url)
    showTextDialog(news , text)
    Menu()
